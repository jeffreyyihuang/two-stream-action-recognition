...
class motion_dataset(Dataset):  
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            self.clips_idx = randint(5,int(nb_clips))
        elif self.mode == 'val':
...
class Motion_DataLoader():
    def __init__(self, BATCH_SIZE, num_workers, in_channel, data_path, dic_path):
        ...
        for key in dic_frame:
            ...
            self.dic_nb_frame[video]= dic_frame[key]
    ...
    def test_video_segment_labeling(self):
        ...
        for video in self.test_video:
            ...
            sampling_interval = int((self.dic_nb_frame[video]-self.in_channel)/nb_test_frame)
            #print self.dic_nb_frame[video], sampling_interval*25
            for index in range(nb_test_frame):
                clip_idx = 5+ index*sampling_interval
                key = video + '-' + str(clip_idx)
                #print key, self.dic_nb_frame[video]-5
    ...
    def train_video_labeling(self):
        ...
        for video in self.train_video:
            ...
            nb_clips = self.dic_nb_frame[video]-self.in_channel
